...
class Minions(object):
    def __init__(self, spec, port=DEFAULT_PORT,
...
class Overlord(object):
    ...
    def setup_ssl(self, client_key=None, client_cert=None, ca=None):
        ...
        fd_key = '%s/etc/pki/certmaster/%s.pem' % (conf_dir, myname)
        fd_crt = '%s/etc/pki/certmaster/%s.cert' % (conf_dir, myname)
        ...
        # otherwise fall through our defaults
            ...
            raise Func_Client_Exception, 'Cannot read ssl credentials: ssl, cert, ca. '+\
                  'Ensure you have permission to read files in %s/etc/pki/certmaster/ directory.' % conf_dir
        ...
        if not self.noglobs:
            if self.nforks > 1 or self.async:
                ...
                if not self.async:
                    ...
                    results = forkbomb.batch_run(minionurls, process_server, int(self.nforks))
                else:
