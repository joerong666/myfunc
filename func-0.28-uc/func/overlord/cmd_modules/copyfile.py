...
class CopyFile(base_command.BaseCommand):
    ...
    def addOptions(self):
        ...
        self.parser.add_option("", "--remotepath", dest="remotepath",
                               action="store")
        self.parser.add_option("-b", "--bufsize", dest="bufsize",
                               action="store")
    ...
    def do(self, args):
        ...
        if not self.options.bufsize:
            return self.overlord_obj.local.copyfile.send(self.options.filename, self.options.remotepath)
        else:
            return self.overlord_obj.local.copyfile.send(self.options.filename, self.options.remotepath, int(self.options.bufsize))
