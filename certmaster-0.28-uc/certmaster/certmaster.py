...
CERTMASTER_CONFIG = "%s/etc/certmaster/certmaster.conf" % conf_dir
...
class CertMaster(object):
    ...
    def wait_for_cert(self, csrbuf, with_triggers=True):
        ...
        if with_triggers:
            self._run_triggers(requesting_host, '%s/var/lib/certmaster/triggers/request/pre/*' % conf_dir)
        ...
        if os.path.exists(certfile):
            ...
            if with_triggers:
                self._run_triggers(requesting_host, '%s/var/lib/certmaster/triggers/request/post/*' % conf_dir)
        ...
        if self.cfg.autosign:
            ...
            if with_triggers:
                self._run_triggers(None, '%s/var/lib/certmaster/triggers/request/post/*' % conf_dir)
            return True, cert_buf, cacert_buf
    ...
    def remove_this_cert(self, hn, with_triggers=True):
        ...
        if not csrs and not certs:
            ...
            return
        if with_triggers:
            self._run_triggers(hn, '%s/var/lib/certmaster/triggers/remove/pre/*' % conf_dir)
        for fn in csrs + certs:
            ...
            os.unlink(fn)
        if with_triggers:
            self._run_triggers(hn, '%s/var/lib/certmaster/triggers/remove/post/*' % conf_dir)
    ...
    def sign_this_csr(self, csr, with_triggers=True):
        ...
        requesting_host = self._sanitize_cn(csrreq.get_subject().CN)
        if with_triggers:
            self._run_triggers(requesting_host, '%s/var/lib/certmaster/triggers/sign/pre/*' % conf_dir)
        ...
        if with_triggers:
            self._run_triggers(requesting_host, '%s/var/lib/certmaster/triggers/sign/post/*' % conf_dir)
...
def main(argv):
    ...
    cm = CertMaster('%s/etc/certmaster/certmaster.conf' % conf_dir)

    if "--version" in sys.argv or "-v" in sys.argv:
        print >> sys.stderr, file("%s/etc/certmaster/version" % conf_dir).read().strip()
        sys.exit(0)

    if "daemon" in argv or "--daemon" in argv:
        utils.daemonize("%s/var/run/certmaster.pid" % conf_dir)
    else:
        print "serving...\n"
